...
export const Canvas: React.FC<CanvasProps> = ({
  ...
  return (
    <div className="flex-1 relative min-h-0 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
      ...
      >
        {/* Canvas workspace - fixed size, horizontally scrollable */}
        <div
          className="relative bg-white dark:bg-slate-800 mx-auto my-4 shadow-xl border border-slate-200 dark:border-slate-600 rounded-xl overflow-visible transition-all duration-300"
          style={{
            width: '800px',
            height: '600px',
            minWidth: '800px'
          ...
          {elements.length === 0 && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none p-4">
              <div className="text-center max-w-md">
                <div className="w-16 h-16 bg-gradient-to-br from-blue-400 via-purple-500 to-pink-500 rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg">
                  <span className="text-2xl text-white">✨</span>
                </div>
                <h3 className="text-xl font-bold text-slate-700 dark:text-slate-300 mb-2">Start Building Your Website</h3>
                <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed">Drag components from the left panel to create your amazing website. Add text, images, buttons and more!</p>
              </div>
            </div>
          )}
